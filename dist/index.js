...
function isRateLimited(phone) {
    ...
    if (count > 20) {
        console.log('[rate] limit hit for', phone);
        return true;
    }
    return false;
}
...
function startPolling() {
    ...
    if (!myPhone) {
        console.error('[poll] MY_PHONE not set');
        return;
    }
    ...
    setInterval(async () => {
        ...
        for (const msg of fresh) {
            ...
            if (isRateLimited(msg.from_number))
                continue;
...
app.get('/', (_req, res) => res.send('grade-brain is running'));
app.post('/webhook', async (req, res) => {
    ...
    if (!from_number || (!content && !media_url))
        return;
    if (from_number !== process.env.MY_PHONE)
        return;
    if (isRateLimited(from_number))
        return;
