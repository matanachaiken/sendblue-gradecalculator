...
import { getUserState, setUserState, setPendingConfirmation, clearPendingConfirmation, getAllClasses, getClass, saveClass, deleteClass, getConfig, saveConfig, setEnvVar, resetClasses, resetAll, saveLastAction, getLastAction, clearLastAction, } from './storage.js';
import { calcCurrentGrade, calcNeeded, calcBestPossible, calcGPA, getLetterGrade, } from './grades.js';
...
async function handlePendingConfirmation(from, pc, answer, state) {
    ...
    switch (pc.type) {
        case 'curve_type': {
            const className = pc.data.className;
            let curveType = null;
            if (/^(1|flat)/.test(lower))
                curveType = 'flat';
            else if (/^(2|mean|scale)/.test(lower))
                curveType = 'mean';
            else if (/^(3|norm)/.test(lower))
                curveType = 'norm';
            else if (/^(4|none|no)/.test(lower))
                curveType = 'none';
            if (!curveType) {
        ...
        case 'canvas_course': {
            ...
            if (!isNaN(idx) && idx >= 0 && idx < courses.length) {
                ...
                selectedCourse = courses.find(c => c.name.toLowerCase().includes(lower) || lower.includes(c.name.toLowerCase().split(':')[0].trim()));
            }
            if (!selectedCourse) {
                setPendingConfirmation(from, pc);
        ...
        case 'confirm_delete': {
            const classKey = pc.data.classKey;
            if (isYes) {
        ...
        case 'confirm_intent': {
            const confirmedIntent = pc.data.intent;
            if (isYes) {
                setUserState(from, { step: 'idle', pendingClass: null });
        ...
        case 'confirm_undo': {
            ...
            switch (last.type) {
                ...
                case 'class_average_saved': {
                    ...
                    if (cd) {
                        if (last.field === 'curve.classAvg') {
                            if (last.previousValue == null) {
                                delete cd.curve.classAvg;
                            }
                            else {
                                cd.curve = { ...cd.curve, classAvg: last.previousValue };
                            }
                        }
                        else if (last.field === 'curve.median') {
                            if (last.previousValue == null) {
                                delete cd.curve.median;
                            }
                            else {
                                cd.curve = { ...cd.curve, median: last.previousValue };
                            }
                        }
                            ...
                            if (!cd.classAverages)
                                cd.classAverages = {};
                            if (last.previousValue == null) {
                                delete cd.classAverages[catKey];
                            }
                            else {
                                cd.classAverages[catKey] = last.previousValue;
                            }
                        }
                        saveClass(last.classKey, cd);
                    }
                ...
                case 'norm_grade_saved': {
                    ...
                    if (cd) {
                        if (last.previousValue == null) {
                            delete cd.curve.mappedGrade;
                        }
                        else {
                            cd.curve = { ...cd.curve, mappedGrade: last.previousValue };
                        }
                        saveClass(last.classKey, cd);
                    }
...
async function handleSetupFlow(from, text, state) {
    ...
    if (step === 'awaiting_syllabus') {
        ...
        if (!parsed?.categories?.length) {
            await sendMessage(from, "Couldn't find grading categories. Try pasting just the breakdown section (e.g. \"Midterm 30%, Final 40%\").");
            return;
        }
    ...
    if (step === 'awaiting_canvas_token') {
        let parsedState = {};
        try {
            parsedState = JSON.parse(pendingClass || '{}');
        }
        catch {
            parsedState = {};
        }
        ...
        try {
            ...
            if (e.message === 'CANVAS_AUTH_ERROR') {
                ...
                await sendMessage(from, "That token didn't work. Make sure you copied the full token and try again. (Or reply \"skip\" to set up Canvas later.)");
            }
            else {
    ...
    if (step === 'awaiting_assignment_classification') {
        ...
        if (lowerClass === 'skip') {
            if (!classData.canvasAssignmentMap)
                classData.canvasAssignmentMap = {};
        ...
        if (queue.length > 0) {
            ...
            const skipNote = skippedCount > 0
                ...
                : '';
            if (skipNote)
                await sendMessage(from, skipNote);
            await showGrade(from, classKey);
        }
        return;
    }
...
async function handleIntent(from, text, intent) {
    ...
    if (action === 'multi') {
        ...
        const batchClassKeys = [...new Set(intent.intents.filter((i) => 'classKey' in i && !!i.classKey).map(i => i.classKey))];
    ...
    if (action === 'update_syllabus') {
        ...
        await sendMessage(from, `Paste the updated grading breakdown for ${classData.name}. Existing grades will be kept.`);
        return;
    }
    // ── Log a grade ───────────────────────────────────────────────────────────
    ...
    if (action === 'help') {
        ...
        await sendMessage(from, 'grade-brain commands:\n' +
...
async function performCanvasSync(from, classKeys) {
    ...
    for (const classKey of classKeys) {
        ...
        try {
            ...
            const e = err;
            if (e.message === 'CANVAS_AUTH_ERROR') {
                await sendMessage(from, 'Canvas token expired. Generate a new one:\nCanvas → Account → Settings → Approved Integrations → New Access Token\n\nThen text "connect canvas" to reconnect.');
                return;
            }
    ...
    if (totalSynced === 0 && ambiguousToAsk.length === 0) {
        ...
        if (totalUngraded > 0)
            reply += ` (${totalUngraded} ungraded skipped)`;
...
async function showGrade(from, classKey) {
    ...
    const catLines = breakdown.map(b => {
        ...
        if (b.count > 1)
            line += ` (avg of ${b.count})`;
        if (b.droppedCount > 0)
            line += ` (${b.droppedCount} lowest dropped)`;
        return line;
    });
    const targets = [
        { pct: 93, label: 'A' },
        ...
        if (classData.curve?.type === 'norm') {
            ...
            for (const t of targets) {
                ...
                if (needed === null)
                    break;
                if (needed <= 0) {
                    neededLines.push(`${t.label}: guaranteed`);
                    break;
                }
                if (needed > 100) {
                    neededLines.push(`${t.label}: not possible (need ${needed})`);
                    break;
                }
...
async function showAllGrades(from) {
    ...
    const lines = keys.map(key => {
        ...
        if (!result)
            return `${c.name}${badge}: no grades yet`;
...
async function processParsedSyllabus(from, pendingClass, parsed, forceRestart = false) {
    ...
    if (isUpdate) {
        saveLastAction({ type: 'syllabus_updated', classKey, className: pendingClass || classKey, previousClassData: JSON.parse(JSON.stringify(existing)) });
    }
    saveClass(classKey, classData);
    if (!isUpdate) {
