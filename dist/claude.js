...
const IMAGE_PROMPT = (className) => `Extract the grading breakdown from this syllabus image${className ? ` for ${className}` : ''}.
...
function mockClassifyIntent(message, classes) {
    ...
    if (m)
        return { action: 'new_class', className: m[1].trim() };
    ...
    if (m)
        return { action: 'check_grade', classKey: matchClass(m[1]) };
    // show all
    if (/show all|all.*grades|all.*classes/.test(lower))
        return { action: 'show_all' };
    // update syllabus for [class]
    m = lower.match(/update syllabus(?:\s+for)?\s+(.+)/);
    if (m)
        return { action: 'update_syllabus', classKey: matchClass(m[1]) || '' };
    // sync / connect canvas
    if (/sync canvas|update grades/.test(lower))
        return { action: 'sync_canvas' };
    if (/connect canvas|setup canvas|login/.test(lower))
        return { action: 'connect_canvas' };
    // delete [class]
    m = lower.match(/^delete\s+(.+)/);
    if (m)
        return { action: 'delete_class', classKey: matchClass(m[1]) || '' };
    if (/^help$/.test(lower))
        return { action: 'help' };
    if (/^reset$/.test(lower))
        return { action: 'reset' };
    if (/^(undo|go back|nevermind|oops|back)$/.test(lower))
        return { action: 'undo' };
    if (/manual|enter manually|add manually/.test(lower))
        return { action: 'enter_manually', classKey: matchClass(lower) };
    // show gpa
    if (/\bgpa\b|grade point/.test(lower))
        return { action: 'show_gpa' };
    // [class] is N credits
    m = lower.match(/^(.+?)\s+is\s+(\d+)\s+credits?/);
    if (m)
        return { action: 'set_credits', classKey: matchClass(m[1]) || '', credits: Number(m[2]) };
