...
function getData() {
    if (!existsSync(DATA_FILE))
        return { classes: {}, userStates: {}, config: {} };
...
await testA('Canvas resync offer — undo canvas link triggers resync question', async () => {
    ...
    const data = getData();
    data.classes['bio 101'].lastSyncedAt = undefined;
    data.lastAction = {
        type: 'canvas_linked',
        classKey: 'bio 101',
        className: 'Bio 101',
...
await testA('"what if I got 90 on bio final" → hypothetical no crash', async () => {
    ...
    try {
        mute();
        await say('what if I got 90 on bio final');
        unmute();
    }
    catch {
        ok = false;
...
await testA('Plain number with one norm class missing median → norm_median_from_number pending', async () => {
    ...
    await say('Homework 50%, Final 50%');
    await say('3');
    await say('no'); // norm curve, no canvas
