...
function getEffectiveWeights(classData) {
    ...
    if (dynamicWeights?.type === 'best_worst') {
        ...
        if (scoreA !== null && scoreB !== null) {
            ...
            if (result[keyA])
                result[keyA] = { ...result[keyA], note: pendingNote };
            if (result[keyB])
                result[keyB] = { ...result[keyB], note: pendingNote };
        }
    }
    return result;
}
...
function applyDropLowest(grades, dropLowest) {
    if (!dropLowest || grades.length <= dropLowest)
        return grades;
...
export function calcGPA(classes) {
    ...
    for (const classData of Object.values(classes)) {
        ...
        if (gpaPoints === undefined)
            continue;
        if (!classData.creditHours) {
            missing.push(classData.name);
            continue;
        }
        included.push({ name: classData.name, letter, gpaPoints, creditHours: classData.creditHours });
    }
    if (included.length === 0)
        return { gpa: null, included, missing, noGrades };
