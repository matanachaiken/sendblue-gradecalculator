// canvas.js — Canvas LMS API, scores only.
//
...
async function canvasGet(baseUrl, token, path, params = {}) {
  ...
  try {
    ...
    if (err.response?.status === 401) throw new Error('CANVAS_AUTH_ERROR');
    throw err;
  }
}
 ...
 * maps these scores to syllabus categories using batchMatchAssignments() in claude.js.
 *
 * @param {string} baseUrl  - Canvas base URL
 * @param {string} token    - Personal access token
 * @param {number} courseId - Canvas course ID
 * @returns {Array<{ id: string, name: string, groupName: string, percentage: number }>}
 */
